...
### HTML Template Setup
Your HTML template MUST include the script and a data-init element:
...
| `data-init` | `data-init="@get('/')"` | Initialization trigger |
...
1. **Datastar Script Setup**
   ...
   - Ensure init element: `<div data-init="@get('/', {retryMaxCount: 1})"></div>`
...
**Cause:** Missing data-init or malformed action
**Fix:**
1. Ensure `<div data-init="@get('/path')">` present
2. Check action syntax: `@post('/path')` not `@post(/path)`
