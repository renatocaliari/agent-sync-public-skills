...
### The Problem
Datastar v1.0.0 requires:
1. A `data-init` element with a **valid action**
2. The STANDARD datastar.js (not aliased version)
...
<!-- In your HTML <body> - MUST have a valid action! -->
<div data-init="@get('/', {retryMaxCount: 1})"></div>
```

**Critical:** `data-init` **MUST** have a valid HTTP action like `@get('/')` or `@post('/api/...')`. Empty `data-init` will NOT work!

### Why This Happens
- The Datastar module registers plugins via `g()` function
- After first plugin registers, it schedules `ln()` initialization via `setTimeout`
- `ln()` sets up the MutationObserver that processes `data-*` attributes
- Without `data-init`, the initialization either doesn't trigger or doesn't complete properly
...
| Clicks not triggering | Missing `data-init` | Add `<div data-init="@get('/')"></div>` |
