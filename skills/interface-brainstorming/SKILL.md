...
description: |
  Generate distinct UI/UX interface proposals for any conceptual product solution.
  Activate this skill automatically or when any of these signals appear:
  - Keywords / intents: "UI", "interface", "screen", "wireframe", "UX", "design alternatives", "explore visual", "trade-offs UI", "frontend options"
  - Workflow stage: during/after brainstorming, when writing plans involving visual/user-facing elements, or when UI decisions are ambiguous
  - Whenever the task involves user interface, experience, layout, or visual flow choices
---
...
- Examples: proposal selection (A/B/C/D), refinement options, next steps, etc.
...
- Generate 3–4 distinct proposals (Radical Simplicity, Conventional, Vanguard, Paradigm Shift)
- Include ASCII wireframes / flow diagrams where helpful
- Provide clear trade-offs table or list (effort, impact, risks, maintainability)
- Use tags: [UI-PROPOSAL-1] [TRADE-OFFS] [RECOMMENDATION]
- After output, use `question` tool to ask for next step:
  - "Select a proposal to refine" (A/B/C/D options)
  - "Integrate selected proposal into plan"
  - "I want you to recommend based on context"
- Fallback (no question tool): "What next? (respond: select proposal A/B/C/D, refine [letter], integrate, or recommend)"

## Generated Output Alignment
Any features, code, UI proposals, scopes, or plans produced by this skill must adhere to the core generation principles: KISS, DRY, Convention over Configuration (sensible defaults), Progressive Disclosure (simple defaults with overrides for advanced), and polymorphism only when beneficial.


## Portability Note
Self-contained skill. Compatible with any agent that can call sub-skills or detect UI-related intents.
